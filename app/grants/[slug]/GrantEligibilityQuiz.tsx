...
function buildFromObject(c: EligibilityCriteria): Question[] {
  ...
  if (c.max_employees) questions.push({
    kind: "yesno", id: "employees", qualifyingAnswer: "yes",
    failReason: `Business must have no more than ${c.max_employees} employees`,
    label: `Does your business have ${c.max_employees} or fewer employees?`,
  ...
  if (c.requires_us_ownership) questions.push({
    ...
    label: "Is your business at least 51% owned and controlled by US citizens?",
    meaning: "Majority ownership means US citizens hold more than half the equity AND have operational control. Foreign parent companies with US subsidiaries must still meet this test.",
    proof: "Articles of incorporation or LLC operating agreement showing ownership percentages and member names. If a trust holds shares, the trust documents must identify the beneficial owners as US citizens.",
  })
  if (c.requires_us_business) questions.push({
    kind: "yesno", id: "us_business", qualifyingAnswer: "yes",
    failReason: "Must be a US-registered business",
    label: "Is your business registered and operating in the United States?",
    meaning: "Your business must be legally formed and principally operating in the US — incorporated, LLC, partnership, or sole proprietorship registered with a US state.",
    proof: "State business registration certificate (Articles of Incorporation, Certificate of Formation, or DBA filing) plus your EIN confirmation letter from the IRS (CP-575).",
  })
  if (c.requires_minority_owned) questions.push({
    kind: "yesno", id: "minority_owned", qualifyingAnswer: "yes",
    failReason: "Business must be at least 51% minority-owned and operated",
    label: "Is your business at least 51% owned and operated by a minority?",
    meaning: "Qualifying minorities include: African American, Hispanic American, Asian American, Native American, and Pacific Islander individuals. The majority owner must also manage day-to-day operations.",
    proof: "NMSDC certification is the most recognized proof. Many programs also accept self-certification with: a copy of your ID, business ownership documents showing your name and ownership percentage, and a statement confirming you control daily operations.",
  })
  if (c.requires_woman_owned) questions.push({
    kind: "yesno", id: "woman_owned", qualifyingAnswer: "yes",
    failReason: "Business must be at least 51% woman-owned and operated",
    label: "Is your business at least 51% owned and operated by a woman?",
    meaning: "The majority owner must be female AND must control day-to-day operations and long-term decisions.",
    proof: "SBA WOSB certification or third-party certifier certificate, OR self-certification with: birth certificate or passport, business ownership documents showing your name and percentage, and a signed statement that you control operations.",
  })
  if ((c as any).requires_rural_business) questions.push({
    kind: "yesno", id: "rural_business", qualifyingAnswer: "yes",
    failReason: "Business must be located in a rural area",
    label: "Is your business located in a rural area?",
    meaning: "USDA defines rural as areas outside urbanized areas with populations under 50,000. Use the USDA's eligibility map at eligibility.sc.egov.usda.gov to verify your exact address.",
    proof: "Your business address is verified against USDA maps — bring your lease or ownership documents for the business location.",
  })
  if ((c as any).requires_research_partnership) questions.push({
    kind: "yesno", id: "research_partnership", qualifyingAnswer: "yes",
    failReason: "Must have a formal research partnership with a US nonprofit research institution",
    label: "Do you have a formal research partnership with a US nonprofit research institution?",
    meaning: "You need an active collaboration agreement with a US university, federally funded research center, or qualifying nonprofit research organization. The partner must perform at least 30% of the R&D work.",
    proof: "A signed Cooperative R&D Agreement (CRADA) or MOU with the research institution, plus the institution's IRS 501(c)(3) determination letter.",
  })
  if ((c as any).requires_phase_1_completion) questions.push({
    kind: "yesno", id: "phase1_done", qualifyingAnswer: "yes",
    failReason: "Must have completed a Phase I award from this program",
    label: "Have you successfully completed a Phase I award from this program?",
    meaning: "Phase II grants require a successful Phase I — the funding agency must confirm technical merit and recommend Phase II continuation. Phase I and Phase II are different competitions.",
    proof: "Your Phase I final technical report and award documentation. Contact your Phase I program officer to confirm your Phase II eligibility before applying.",
  })
  if ((c as any).requires_underserved_market) questions.push({
    kind: "yesno", id: "underserved", qualifyingAnswer: "yes",
    failReason: "Business must primarily serve an underserved or low-income market",
    label: "Does your business primarily serve an underserved or low-income community?",
    meaning: "Underserved markets include low-income communities, rural areas, minority communities, and areas with limited access to traditional banking or capital.",
    proof: "Business plan narrative describing your target market. Census tract data showing your service area's income levels, or a letter from a CDFI or community development organization supporting your application.",
  })
  if (c.industries?.length) questions.push({
    kind: "yesno", id: "industry", qualifyingAnswer: "yes",
    failReason: `Business must operate in: ${c.industries.join(", ")}`,
    label: `Does your business operate in one of these sectors: ${c.industries.map(i => i.charAt(0).toUpperCase() + i.slice(1)).join(", ")}?`,
    meaning: `Your primary business activity must fall within one of these sectors. NAICS codes are used to classify industries — look up yours at census.gov/naics to confirm you qualify.`,
    proof: "Your NAICS code as registered with your state or in your federal SAM.gov registration. Your business license or industry certifications may also demonstrate this.",
  })
